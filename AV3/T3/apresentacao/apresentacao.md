...
[Sorvedouro T]
```
* **Fonte -> tamanho:** limita o estoque (N/6 unidades por tamanho).
* **Tamanho -> voluntário:** capacidade = 1 (se o voluntário aceita o tamanho).
* **Voluntário -> sorvedouro:** capacidade = 1 (garante 1 camiseta por pessoa).
...
* **Grafo residual:** arestas reversas permitem "desfazer" alocações.
  * Se V2 tomou o tamanho L, mas V3 precisa de L exclusivamente, o algoritmo redireciona o fluxo pela aresta reversa.
...
* Se fluxo máximo < M -> **NO** (falta de estoque em tamanhos compatíveis).
...
**Conteúdo do Slide:**
* **Complexidade de tempo:** O(E * f*) onde f* <= M <= 30.
  * No pior caso: 96 x 30 = 2.880 operações (execução instantânea).
* **Por que não Edmonds-Karp (BFS)?** Edmonds-Karp teria custo teórico de O(V * E^2) ≈ 350.000 operações. Para fluxos pequenos, o Ford-Fulkerson com DFS é mais simples de implementar e extremamente rápido.
...
* **Falas:**
  > "Para concluir, analisando a complexidade, a nossa rede possui no máximo 38 vértices e 96 arestas. Como o fluxo máximo é pequeno (f* <= 30), o Ford-Fulkerson clássico com busca por DFS tem complexidade O(E * f*), executando em menos de 3 mil operações — muito mais rápido e simples do que Edmonds-Karp neste caso.
